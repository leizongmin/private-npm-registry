...
module.exports = function (done) {
  ...
  dashRouter.put('/user/org.couchdb.user*', co.wrap(function* (req, res, next) {
    $.logger.log('user login: %s', req.body.name);
    try {
      const user = yield $.service.call('user.get', { name: req.body.name });
      const responseOK = function (user) {
      ...
      if (user) {
        ...
        // sign up
        const newUser = yield $.service.call('user.create', req.body);
        responseOK(newUser);
      }
    } catch (err) {
      return res.json($.utils.npmError(err.message));
    }
  }));
